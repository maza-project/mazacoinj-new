...
package com.google.zetacoin.uri;
...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "mazacoin";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        if (input.startsWith("mazacoin://")) {
            schemeSpecificPart = input.substring("mazacoin://".length());
        } else if (input.startsWith("mazacoin:")) {
            schemeSpecificPart = input.substring("mazacoin:".length());
        } else {
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the mazacoin: prefix");
